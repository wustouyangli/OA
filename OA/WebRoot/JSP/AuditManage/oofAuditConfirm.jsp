...
<html>
  <head>
    ...
    <title>My JSP 'oofAuditConfirm.jsp' starting page</title>
  ...
  <img src="Image/return.png" style=" position: absolute;left: 10px;top: 10px;" height="30px" width="30px" onclick="history.back()"> <br>
    ...
    <%TbOof m = dao.findById(Integer.parseInt(request.getParameter("id"))); %>
    <%if (m != null){ %> 
    <div class="mainDiv">
    ...
    <label class="lb">出差原因</label><input type="text" name="content" class="txt" disabled="true" size="50" value="<%=m.getContent() %>"> <br>
    ...
    <a class="auditop" href="oofauditupdate.action?id=<%=m.getId()%>&state=1">批准</a> 
    <a class="auditop" href="oofauditupdate.action?id=<%=m.getId()%>&state=2">不批准</a> 
    <a class="auditop" href="oofauditupdate.action?id=<%=m.getId()%>&state=3">暂不审核</a> <br>
    </div>  
    <%} %> 
  </body>
</html>
