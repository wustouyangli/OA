...
<html>
  <head>
    ...
    <title>My JSP 'takeoffAudit.jsp' starting page</title>
  ...
  <img src="Image/return.png" style=" position: absolute;left: 10px;top: 10px;" height="30px" width="30px" onclick="history.back()"> <br>
    ...
    <%}else{ %>
      <a href="JSP/AuditManage/takeoffAudit.jsp?offset=<%=offset-1 %>">[上一页]</a>
    <%} %> 
    <%for (int i = 0; i < group; i++){ %>
       <%if (i == offset){ %>
        <a href="JSP/AuditManage/takeoffAudit.jsp?offset=<%=i %>" id="curPageLink">[<%=i + 1 %>]</a>
       <%}else{ %>
        <a href="JSP/AuditManage/takeoffAudit.jsp?offset=<%=i %>">[<%=i + 1 %>]</a>
       <%} %>
    <%} %>
    <%if (offset == group-1){ %>
      <a href="javascript:return false;" id="nextPageLink">[下一页]</a>
    <%}else{ %>
      <a href="JSP/AuditManage/takeoffAudit.jsp?offset=<%=offset+1 %>">[下一页]</a>
    <%} %>
    </div>
  </body>
</html>
